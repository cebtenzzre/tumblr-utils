...
3. The `jq` extra enables the `--filter` option to filter the downloaded posts with arbitrary
   rules based on their metadata.
4. The `dash` extra enables backing up [dashboard-only blogs](#dashboard-only-blogs).
